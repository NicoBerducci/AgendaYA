...
describe('Épica: Confirmación de Reserva (Tomás Yanardi)', () => {
  ...
  describe('US_033: Validación de datos obligatorios - Email', () => {
    it('Debe devolver error si se ingresa un email vacío o inválido como "usuario@"', () => {
      const vacio = validarEmail('');
      expect(vacio.isValid).toBe(false);
      expect(vacio.errorMessage).toBe('Ingrese un email válido para continuar');

      const invalido = validarEmail('usuario@');
      expect(invalido.isValid).toBe(false);
      expect(invalido.errorMessage).toBe('Ingrese un email válido para continuar');
    });

    it('Debe devolver válido si el email tiene un formato correcto', () => {
      const resultado = validarEmail('tomas.yanardi@gmail.com');
  ...
  describe('US_033: Validación de datos obligatorios - Nombre', () => {
    it('Debe devolver error si el nombre ingresado es inválido (ej. vacío o "a")', () => {
      const vacio = validarNombre('');
      expect(vacio.isValid).toBe(false);
      expect(vacio.errorMessage).toBe('Ingrese un nombre válido para continuar');

      const corto = validarNombre('a');
      expect(corto.isValid).toBe(false);
      expect(corto.errorMessage).toBe('Ingrese un nombre válido para continuar');
    });

    it('Debe devolver válido si el nombre es correcto', () => {
      const resultado = validarNombre('Tomás Yanardi');
      expect(resultado.isValid).toBe(true);
    });
  });

  // PRUEBA 3: correspondiente a US_033 (Teléfono)
  describe('US_033: Validación de datos obligatorios - Teléfono', () => {
    it('Debe devolver error si el teléfono está vacío o es muy corto', () => {
      const vacio = validarTelefono('');
      expect(vacio.isValid).toBe(false);
      expect(vacio.errorMessage).toBe('Ingrese un teléfono válido para continuar');
    });

    it('Debe devolver válido si el teléfono tiene un formato adecuado', () => {
      const resultado = validarTelefono('2604123456');
      expect(resultado.isValid).toBe(true);
    });
  });

  // PRUEBA 4: correspondiente a US_035 (Expiración)
